...
// One expanded case study on the home page, replacing the old compact card.
// Problem, Solution, and Results are all visible without a click — the detail
// page is for depth, not for the basics.
//
// The aside column cascades canvas -> figure -> facts. Every study lands on
// exactly one of the three, so the column is never empty and the four blocks
// stay visually consistent down the page.
export default function WorkBlock({ study, benchmarkFootnote }: WorkBlockProps) {
  const thumbnail = study.figures?.[0];

  return (
    <article className="grid gap-10 border-t border-border pt-12 lg:grid-cols-[1fr_20rem] lg:gap-16">
      <div>
